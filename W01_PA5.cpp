...
대소문자의 구분은 없으며, 출력에서 모든 단어들은 소문자로 변환되어야 한다. 단어들은 사전식 순서로 정렬되어 출력되어야 한다. 
영문 알파벳이 전혀 포함되지 않은 문자열은 그냥 무시한다. 또한 단어의 앞과 뒤에 붙은 쉼표, 마침표, 물음표 등의 알파벳이 아닌 기호들은 제거되어야 한다. 
단, 단어의 중간에 삽입된 문자들은 단어의 일부로 간주한다. 
예를 들어 o'clock이나 brother's 혹은 break*-,!fast--parlour 등은 그 자체로 하나의 단어로 취급한다. 
출력은 화면으로 한다. 
한 줄에 하나의 단어와 그 단어의 등장 빈도를 출력하라. 
단어와 등장빈도 사이에 한 칸을 띄워라. 
모든 단어를 출력하지말고 10개씩 건너띄어서 출력한다. 
즉, 정렬된 순서에서 첫번째 단어, 11번째 단어, 21번째 단어,... 이런 식으로 출력한다.
*/
